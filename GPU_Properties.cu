...
int main() {
    ...
    for (int i = 0; i < deviceCount; ++i) {
        ...
        // Properites beyond this are printed by sumit
        std::cout << "\n\nSOME EXTRA PROPERTIES " << "\n\n";
        std::cout << "Maximum number of resident threads per SM: " << prop.maxThreadsPerMultiProcessor << "\n";
        std::cout << "Total registers per block: " << prop.regsPerBlock << "\n";
        std::cout << "Registers per SM " << prop.regsPerMultiprocessor << "\n"; 
        std::cout << "Maximum block dimensions (x,y,z): " << prop.maxThreadsDim[0] << ", " << prop.maxThreadsDim[1] << ", " << prop.maxThreadsDim[2] << "\n";
        std::cout << "Maximum grid dimensions (x,y,z): " << prop.maxGridSize[0] << ", " << prop.maxGridSize[1] << ", " << prop.maxGridSize[2] << "\n";
        std::cout << "GPU clock rate: " << prop.clockRate/1000 << "MHz\n";
        std::cout << "Memory bus width: " << prop.memoryBusWidth/8 << "Bytes\n";
        std::cout << "Memory clock rate: " << prop.memoryClockRate/1000 << "MHz\n"; 
        std::cout << "Warp Size: "<<prop.warpSize<<"\n";
        std::cout << "Maximum memory pitch: " << prop.memPitch << "\n"; 
        std::cout << "Multiprocessors L2 Cache: " << prop.l2CacheSize/1024 << "KB\n";
        std::cout << std::endl; 

        size_t freeMem, totalMem;
        cudaMemGetInfo(&freeMem, &totalMem);
        std::cout << "Free: " << freeMem / (1024 * 1024) << " MB\n";
        std::cout << "Total: " << totalMem / (1024 * 1024) << " MB\n";
